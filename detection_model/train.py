...
from utils.general import (
    ...
    not_increment_dir, calc_img_weights, update_bn)
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if USE_SWA:
        ...
        swa_scheduler = SWALR(optimizer, swa_lr=0.05, anneal_epochs=5, anneal_strategy='cos')
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(train_ann_file, train_root, imgsz, batch_size, gs, opt, hyp=hyp, augment=True,
                                            cache=opt.cache_images, rect=opt.rect, local_rank=rank,
                                            world_size=opt.world_size, image_weights=data_dict.get('image_weights', False),
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        #         image_weights = calc_img_weights(dataset.labels)
        #         dataset.indices = np.random.choice(dataset.n, size=dataset.n, replace=True, p=image_weights)
        #     # Broadcast if DDP
        #     if rank != -1:
        ...
        if rank in [-1, 0]:
            ...
            if final_epoch or (epoch > 0 and epoch % opt.test_freq == 0):  # Calculate mAP  not opt.notest or
    ...
    if USE_SWA:
        ...
        # copy the parameter from swa_model to model
        ...
        if rank in [-1, 0]:  # save parameter for SWA
            save_model(model, results_file, epoch, best_fitness, optimizer, opt,
                       last.replace('.pt', '_{:03d}.pt'.format(epoch)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fold', type=str, default='', help="which fold of K fold cross-validation.")
    parser.add_argument('--keep_no_anno_img', action='store_true', help='whether using images with no annotations to train')
    parser.add_argument('--high_thr', type=float, default=None, help='high threshold of two-threshold method')
    parser.add_argument('--resam_normal_freq', type=int, default=-1, help='how many epoch to resample the normal image ids')
