...
def test(data,
    ...
    if not training:
        ...
        test_ann_file = data['{}_ann'.format(opt.task)].format(opt.rad_id, opt.fold)  # path to val/test json annotations
        test_img_root = data['{}_img_root'.format(opt.task)].format(opt.rad_id, opt.fold)   # path to val/test images
        dataloader = create_dataloader(test_ann_file, test_img_root, imgsz, batch_size, model.stride.max(), opt,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # Compute loss
            # if training and model.hyp['fl_gamma'] <= 0:  # if model has loss hyperparameters
        ...
        for si, pred in enumerate(output):
            ...
            if nl:
                ...
                # update predictions using high_thr for two-threshold method.
                if high_thr is not None and (pred[:, 4] >= high_thr).sum() < 1:
                    pred = pred.new_tensor([[0, 0, 1, 1, 1, 14]])
