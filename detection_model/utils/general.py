...
def calc_img_weights(labels):
    """ calculate image sampling weights to overcome class-imbalance
        labels (list[np.ndarray]): [k, 5] (cls_id, cx, cy, bw, bh)
    """
    ...
    for i in range(len(labels)):
        box_info = labels[i]
        img_cls = set([int(cls) for cls in box_info[:, 0].astype(np.int)])
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        p_t = true * (1 - pred_prob) + (1 - true) * pred_prob
        alpha_factor = true * self.alpha + (1 - true) * (1 - self.alpha)
        loss = self.loss_fcn(pred, true) * alpha_factor * p_t.pow(self.gamma)
...
def compute_loss(p, targets, model, return_each=False):  # predictions, targets, model
    ...
    ce_plus_focal = h.get('ce_plus_focal', False)
    if g > 0:
        if not ce_plus_focal:  # focal loss only
            BCEobj = MyFocalLoss(gamma=g).to(device)
        else:  # cross-entropy + focal
            # FLcls = FocalLoss(nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['cls_pw']])).to(device), g)
            FLobj = FocalLoss(nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['obj_pw']])).to(device), g)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if g > 0 and ce_plus_focal:
            lobj += FLobj(pi[..., 4], tobj) * balance[i]  # obj loss
    ...
    if return_each:  # return each of the loss seperately
        return lobj * bs, lcls * bs, lbox * bs

    loss = lbox + lobj + lcls
    return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()


# def compute_loss_ohem(p, targets, model, return_each=False):  # predictions, targets, model
#     """online hard negative mininig"""
#     device = targets.device
#     lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
#     tcls, tbox, indices, anchors = build_targets(p, targets, model)  # targets
#     h = model.hyp  # hyperparameters
...
#             # with open('targets.txt', 'a') as file:
#             #     [file.write('%11.5g ' * 4 % tuple(x) + '\n') for x in torch.cat((txy[i], twh[i]), 1)]
#         # compute the hard negative mask
#         with torch.no_grad():
#             tar = torch.zeros_like(pi[..., 0], device=device)
#         lobj += BCEobj(pi[..., 4], tobj) * balance[i]  # obj loss
#
#     s = 3 / np  # output count scaling
#     lbox *= h['giou'] * s
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        if nt:
            ...
            # Offsets
            gxy = t[:, 2:4]  # grid xy
            gxi = gain[[2, 3]] - gxy  # inverse, ow - cx, oh - cy
            j, k = ((gxy % 1. < g) & (gxy > 1.)).T
            l, m = ((gxi % 1. < g) & (gxi > 1.)).T
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    Args:
        prediction (Tensor): [bs, h*w*na, no] all layer output are concanated together,
                              each row is (tx, ty, tw, th, pc, cls_scores)
    Returns:
...
def not_increment_dir(dir, comment=''):
    # Increments a directory runs/exp1 --> runs/exp2_comment
    dir = str(Path(dir))  # os-agnostic
...
def update_bn(loader, model, device=None):
    ...
    for input in tqdm(loader, total=len(loader)):
